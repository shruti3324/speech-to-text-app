...
def transcribe_audio_file():
    ...
    try:
        ...
        with sr.AudioFile(file_path) as source:
            ...
            apply_noise_mode(recognizer, source, mode)
...
# ------------------- Noise Mode Handler -------------------
def apply_noise_mode(recognizer, source, mode):
    """Adjust microphone sensitivity based on noise environment"""
    if mode == "Home":
        set_status("Calibrating: Home noise reduction...")
        recognizer.adjust_for_ambient_noise(source, duration=0.5)
    elif mode == "Office":
        set_status("Calibrating: Office noise reduction...")
        recognizer.adjust_for_ambient_noise(source, duration=1.0)
    elif mode == "Other":
        set_status("Calibrating: General noise reduction...")
        recognizer.adjust_for_ambient_noise(source, duration=0.8)
    elif mode == "Car":
        set_status("Calibrating: Car environment...")
        recognizer.adjust_for_ambient_noise(source, duration=1.2)
    elif mode == "Cafe":
        set_status("Calibrating: Cafe environment...")
        recognizer.adjust_for_ambient_noise(source, duration=1.5)
    elif mode == "Outdoor":
        set_status("Calibrating: Outdoor environment...")
        recognizer.adjust_for_ambient_noise(source, duration=2.0)
    elif mode == "Studio":
        set_status("Calibrating: Studio (quiet)...")
        recognizer.adjust_for_ambient_noise(source, duration=0.3)
    elif mode == "College":
        set_status("Calibrating: College environment...")
        recognizer.adjust_for_ambient_noise(source, duration=1.3)
...
noise_options = ["Home", "Office", "Other", "Car", "Cafe", "Outdoor", "Studio", "College"]  # ✅ Added "College"

noise_combo = tb.Combobox(
    noise_frame,
    textvariable=noise_mode_var,
    values=noise_options,
    font=("Segoe UI", 11),
    width=15,
    state="readonly"
)
